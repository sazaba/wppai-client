...
import { ChevronLeft, ChevronRight, Plus, Users, Star } from 'lucide-react';
...
const DAYS_DATA = [
  ...
  { day: 14, status: 'active', citas: 3, hasAppointment: true }, // Día ocupado
            ...
            <div className="flex flex-col xl:flex-row justify-between items-center p-4 sm:p-6 border-b border-white/5 bg-[#14171F] gap-4">
                ...
                <div className="flex items-center gap-2 w-full xl:w-auto justify-between xl:justify-end overflow-x-auto scrollbar-hide">
                    <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-[#1E222B] rounded-lg text-xs text-gray-300 border border-white/5 whitespace-nowrap">
                        <Users size={12} /> Cabinas
                    </div>
                    <div className="flex bg-[#1E222B] rounded-lg p-1 border border-white/5 shrink-0">
                        <button className="px-3 py-1 bg-rose-600 text-white rounded text-xs font-medium shadow-lg shadow-rose-500/20">Mes</button>
                        <button className="px-3 py-1 text-gray-400 hover:text-white text-xs transition">Sem</button>
                    </div>
                    <button className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-rose-600 to-purple-600 rounded-lg text-white text-xs font-bold shadow-lg hover:opacity-90 transition whitespace-nowrap shrink-0">
                        <Plus size={14} /> <span className="hidden sm:inline">Cita</span>
                    </button>
                </div>
            </div>
                    ...
                    {DAYS_DATA.map((d, i) => (
                        <div 
                            ...
                            className={`
                                ...
                                ${d.hasAppointment 
                                    ? 'bg-[#1a1216] border-rose-900/50 hover:border-rose-500/50' 
                                    : 'bg-[#14171F] border-white/5 hover:border-white/10 hover:bg-[#1A1D26]'}
                                ${d.status === 'fuera' ? 'opacity-30' : 'opacity-100'}
                            `}
                        >
                            <div className="flex justify-between items-start">
                                <span className={`text-xs sm:text-sm font-bold ${d.hasAppointment ? 'text-rose-400' : 'text-white'}`}>{d.day}</span>
                                {d.citas > 0 && (
                                    <span className="hidden sm:flex w-1.5 h-1.5 rounded-full bg-rose-500 shadow-[0_0_5px_rgba(244,63,94,0.8)]" />
                                )}
                            </div>

                            <div className="flex-1 flex flex-col justify-center gap-1">
                                {d.citas > 0 ? (
                                    <div className="px-1.5 py-0.5 rounded bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[9px] sm:text-[10px] font-medium truncate text-center">
                                        {d.citas} Citas
                                    </div>
                                ) : (
