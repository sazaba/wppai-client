        ...
        <AnimatePresence>
            ...
            >
                ...
                {Object.entries(STATUS_STYLES).map(([key, style]) => (
                    ...
                    >
                    <style.icon className={clsx("w-3.5 h-3.5 shrink-0", style.color.split(' ')[0])} />
                    <span className="flex-1 truncate">{style.label}</span>
                    {value === key && <Check className="w-3 h-3 text-indigo-400 shrink-0" />}
                    </button>
                ))}
                </div>
            </motion.div>
    ...
    <div className="h-[100dvh] w-full bg-zinc-950 text-white relative overflow-hidden flex flex-col">
      ...
      <div className="flex-1 w-full overflow-y-auto overflow-x-hidden relative z-10 px-4 md:px-8 py-4 md:py-8 whatsapp-scroll">
        <div className="max-w-7xl mx-auto space-y-6 flex flex-col min-h-full">
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-5 border-b border-white/5 pb-6">
            <div className="w-full xl:w-auto">
              <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight flex items-center gap-3">
                <Database className="text-indigo-500 w-6 h-6 md:w-8 md:h-8" />
                Gestión de Leads
              </h1>
              ...
              <div className="flex items-center gap-2 md:gap-6 mt-4 overflow-x-auto pb-1 w-full no-scrollbar mask-gradient-right">
                {[
                  ...
                  { id: 'contacted', label: 'Contactados', icon: CheckCircle2 },
                  { id: 'closed', label: 'Cerrados', icon: XCircle },
                   ...
                   <button 
                      ...
                      className={clsx(
                        "text-sm font-medium px-3 py-1.5 rounded-full border transition-all flex items-center gap-2 whitespace-nowrap shrink-0",
                        viewTab === tab.id 
                          ? tab.id === 'pending' ? 'bg-amber-500/10 border-amber-500/50 text-amber-400'
                          : tab.id === 'contacted' ? 'bg-blue-500/10 border-blue-500/50 text-blue-400'
                          : tab.id === 'closed' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400'
          ...
          {/* GRID DE RESULTADOS */}
          {loading ? (
             <div className="flex-1 flex flex-col items-center justify-center py-20 space-y-4 min-h-[300px]">
               <Loader2 className="animate-spin text-indigo-500 w-10 h-10" />
               <span className="text-zinc-500 text-sm">Sincronizando agenda...</span>
             </div>
          ) : filtered.length === 0 ? (
             <div className="flex-1 flex flex-col items-center justify-center py-20 text-center border border-dashed border-white/5 rounded-3xl bg-white/5 min-h-[300px]">
                    ...
                    >
                        ...
                        <div className="flex justify-between items-start mb-4 pl-3">
                            <div className="flex items-center gap-3 overflow-hidden">
                                <div className="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-indigo-500/20">
                                    {demo.name.charAt(0).toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-base font-bold text-white leading-tight truncate" title={demo.name}>
                                        {demo.name}
                                    </h3>
                            ...
                            <button
                                ...
                                className="p-2 rounded-lg bg-zinc-800/50 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-colors md:opacity-0 md:group-hover:opacity-100 shrink-0"
                            >
                        ...
                        <div className="space-y-3 pl-3 flex-1">
                            ...
                            <div className="flex items-center gap-3 p-2 rounded-lg bg-white/[0.02] border border-white/5">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-sm text-zinc-200 capitalize truncate">
                            ...
                            <div className="flex flex-col gap-2 text-sm">
                                <a href={`mailto:${demo.email}`} className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors truncate">
                                    <Mail className="w-3.5 h-3.5 shrink-0" /> <span className="truncate">{demo.email}</span>
                                </a>
                                <a href={`tel:${demo.phone}`} className="flex items-center gap-2 text-zinc-400 hover:text-emerald-400 transition-colors truncate">
                        ...
                        <div className="flex flex-wrap sm:flex-nowrap items-center justify-between pt-4 mt-2 border-t border-white/5 pl-3 gap-3">
                            ...
                            <a 
                                href={`tel:${demo.phone}`} 
                                className="w-full sm:w-auto flex justify-center items-center p-2.5 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all"
                            >
                                <Phone className="w-4 h-4" />
                                <span className="sm:hidden ml-2 text-xs font-bold uppercase">Llamar</span>
                            </a>
                        </div>
                    </motion.div>
                ))}
