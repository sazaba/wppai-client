...
export default function ChatsPage() {
  ...
  const handleSelectChat = async (chatId: number) => {
    ...
    try {
      ...
      if (chatActual?.estado === 'pendiente') {
        await axios.put(`/api/chats/${chatId}/estado`, { estado: 'en_proceso' }, { headers: { Authorization: `Bearer ${token}` } })
      }
      const res = await axios.get(`/api/chats/${chatId}/messages?page=1&limit=20`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const res = await axios.get(`/api/chats/${activoId}/messages?page=${nextPage}&limit=20`, { headers: { Authorization: `Bearer ${token}` } })
      const mapped = res.data.messages.map((m: any) => ({
        id: m.id,
        externalId: m.externalId,
        from: m.from,
  ...
  const handleSendMessage = async () => {
    ...
    if (chatActual?.estado !== 'cerrado') {
      try {
        ...
        await axios.post(`/api/chats/${activoId}/responder-manual`, { contenido: body }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleCerrarConversacion = async () => {
    if (!activoId) return
    try {
      await axios.put(`/api/chats/${activoId}/estado`, { estado: 'cerrado' }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleCrearConversacion = async (data: { nombre?: string; phone: string }) => {
    try {
      ...
      await axios.put(`/api/chats/${res.data.chat.id}/estado`, { estado: 'en_proceso' }, { headers: { Authorization: `Bearer ${token}` } })
