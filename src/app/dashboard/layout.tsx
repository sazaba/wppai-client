...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // 1. Extraemos 'loading' del AuthContext
  ...
  // 2. CORRECCIÓN PRINCIPAL: Solo redirigir si NO está cargando
  useEffect(() => {
    // Si está cargando, no hacemos nada (esperamos)
    if (loading) return 

    // Si ya terminó de cargar y no está autenticado, ahí sí redirigimos
    if (!isAuthenticated && !isOpenRoute) {
        router.replace("/")
    }
  }, [isAuthenticated, isOpenRoute, router, loading]) // Agregamos 'loading' a dependencias

  useEffect(() => {
    // También protegemos este fetch con 'loading'
    if (loading || !isAuthenticated || isOpenRoute) return
    ...
    fetchStatus()
  }, [isAuthenticated, isOpenRoute, loading])
  ...
  // 3. Renderizar null o un spinner mientras carga el estado de auth
  // Esto evita el "parpadeo" o redirección prematura
  if (loading) return null // O puedes poner un <LoadingScreen /> aquí
