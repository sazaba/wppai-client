...
export type AppointmentConfigValue = {
  ...
  rules?: {
    bookingWindowDays?: number | null;        // ✓ Se mapea a maxAdvanceDays
    maxDailyAppointments?: number | null;     // ⛔ No usado por la IA (KB)
    cancellationWindowHours?: number | null;  // ✓ Se mapea a minNoticeHours
  ...
  kb?: {
    businessOverview?: string | null;          // ⛔ No usado por la IA (KB)
    /** 🔹 NUEVO: arreglo visual de FAQs que se guardará como JSON en DB (kbFAQs) */
    faqs?: Array<{ q: string; a: string }> | null; // ✓ Sí lo usa la IA
    /** Compat: copia JSON string de faqs para backends antiguos */
    faqsText?: string | null; // ✓ Persistencia/compat
    freeText?: string | null; // ⛔ No usado por la IA (KB)
  };

  hours?: AppointmentDay[]; // ⛔ La KB no lo usa (lo usa el motor de disponibilidad si lo tienes)
};
...
export function EsteticaForm({ value, onChange }: Props) {
  ...
  // Defaults UI (solo para campos relevantes a IA)
  useEffect(() => {
    const r = value.rules ?? {};
    const toSet: Partial<NonNullable<typeof value.rules>> = {};
    let changed = false;

    if (r.bookingWindowDays == null) { toSet.bookingWindowDays = 30; changed = true; }       // ✓ IA
    if (r.cancellationWindowHours == null) { toSet.cancellationWindowHours = 12; changed = true; } // ✓ IA

    // ⛔ Oculto: no usado por IA — se comenta para simplificar UI
    // if (r.maxDailyAppointments == null) { toSet.maxDailyAppointments = 0; changed = true; }
    // if (r.depositRequired == null) { toSet.depositRequired = false; changed = true; }
  ...
  return (
    <div className="space-y-10">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input
            ...
            onChange={(e) => patchNested("location", { mapsUrl: e.target.value })}
          />
          {/* ⛔ Oculto: virtualLink no es leído por la IA (KB) hoy
          <Input
            type="text"
            placeholder="Link de videollamada (si aplica)"
            value={value.location?.virtualLink ?? ""}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          */}
          ...
          {/* ⛔ Oculto: no-show policy global no la usa la IA (usa reglas/procedimientos)
          <Field label="Política de inasistencia (No-show)">
            <Textarea
              rows={3}
              placeholder="Ej: Si no asistes sin avisar, se cobrará $X o se pierde el depósito."
          ...
          */}
          ...
          {/* ⛔ Oculto: depósito global no lo usa la IA (usa depositRequired/Amount por procedimiento)
          <div className="md:col-span-3 flex flex-col sm:flex-row gap-4">
            <label className="inline-flex items-center gap-2">
              <input
                id="depositRequired"
      ...
      {/* ====== 5) Mensajes ====== */}
      {/* ⛔ Sección completa oculta: la IA (KB) no consume estos campos hoy
      <Section
        title="Mensajes al cliente"
        subtitle="Plantillas de recordatorio y mensaje posterior a la reserva."
      >
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          ...
          */}
          ...
          {/* 🔹 Editor visual de FAQs (sí lo usa la IA) */}
          <div>
            <div className="mb-2 text-sm font-medium text-slate-200">FAQs</div>
            <FAQEditor
              items={value.kb?.faqs}
            ...
            />
            <div className="mt-2 text-[11px] text-slate-400">
              Se guardan como arreglo JSON interno compatible con la BD.
            </div>
          </div>
